...
const QuizBoard = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await axiosPrivate.get(`/quiz/viva-question?quiz_or_viva_id=${getQuizid()}`);
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        const response = await axiosPrivate.get(`/quiz/viva-options?question_id=${question.id}&quiz_or_viva_id=${getQuizid()}`);
  ...
  return (
    <div className="flex justify-center items-center h-[100vh]  p-10 ">
      ...
      <button className="join-item btn btn-neutral w-20"
        onClick={() => {saveQuiz(question.id)}}
      >
        save
      </button>
